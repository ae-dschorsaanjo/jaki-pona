...
`!`&shy;` ` ah! I got it! Hahaha! (as "a a a")  
`ld`&shy;` ` kin  

__akesi__ ![akesi](pics/akesi.png)
`f`&shy;`1` reptile, amphibian  
`f`&shy;`2` non-cute animal, monster (dialectal)  
_non-cute animal_ definition has been removed in the Toki Pona Dictionary.

__ala__ ![ala](pics/ala.png)
`f`&shy;` ` nothing, no one, null, the lack of (something)  
`i`&shy;` ` deny  
`m`&shy;` ` no, not  
`!`&shy;` ` no!  

__alasa__ ![alasa](pics/alasa.png)
`f`&shy;` ` hunting, searching  
`i`&shy;`1` hunt, forage, gather  
`i`&shy;`2` look for, search determinedly  

__ale/ali__ ![ale/ali](pics/ale.png)
`f`&shy;` ` everything, life, universe  
...
`f`&shy;` ` bottom, below  
`i`&shy;`1` kneel, bow, accept (the terms)  
`i`&shy;`2` defeat, beat, conquer, enslave  
`m`&shy;`1` low, below, down, beneath  
`m`&shy;`2` dependent  

__ante__ ![ante](pics/ante.png)
`f`&shy;` ` change, difference, diversity  
`i`&shy;` ` alter, change, convert, transform  
...
`f`&shy;` ` depression, abuse, shame, dishonor  
`i`&shy;` ` verbal attack, disgrace, depress  
`m`&shy;` ` shameful, dishonored, depressive  
`e`&shy;` ` ike, utala toki  
Added in early 2010, but was not included in _pu_.

__awen__ ![awen](pics/awen.png)
`f`&shy;` ` waiting, staying  
`i`&shy;`1` wait, wait for (with "tawa"), stay, remain  
`i`&shy;`2` keep something unchanged  
`s`&shy;` ` keep  
